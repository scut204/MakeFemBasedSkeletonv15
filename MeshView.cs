                    ...
                    if (currMeshRecord.Skeletonizer != null)
                    {
                        switch (e.Button)
                        {
                            case MouseButtons.Left: 
                                skeHandleIndex = currMeshRecord.Skeletonizer.SelectSkeNode(this.Width, this.Height, currMousePosition);
                                skeHandlepos = currMeshRecord.Skeletonizer.GetNodePos(skeHandleIndex);
                                break;
                            case MouseButtons.Right: 
                                currMeshRecord.Skeletonizer.UnselectNode();
                                skeHandleIndex = -1;
                                skeHandlepos = new Vector3d();
                                break;
                        }
                        isMouseDown = false;
                        this.Refresh();
			...
			switch (Program.currentMode)
			{
                ...
                case Program.EnumOperationMode.PickSkeNode:
                    if (isMouseDown && skeHandleIndex != -1)
					{
						...
						Matrix4d tran = currInverseTransformation * this.movingBall.GetMatrix() * currTransformation;
						//Matrix4d tran = currTransformation * this.movingBall.GetMatrix() * currInverseTransformation;
                        Vector4d q = new Vector4d(skeHandlepos, d);
                        q = tran * (q - skeHandleCenter) + skeHandleCenter;
                        Vector3d qs = new Vector3d(q.x, q.y, q.z);
                        currMeshRecord.Skeletonizer.SetNodePos(qs, skeHandleIndex);
...
//					this.currInverseTransformation = this.currTransformation.Inverse();
                ...
                case Program.EnumOperationMode.PickSkeNode:
                    ...
                    this.currMeshRecord.Skeletonizer.ComputeNodePlanarNormal();
                    this.movingBall.End();
                    this.Refresh();
                    break;                    
			}

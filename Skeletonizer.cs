...
namespace IsolineEditing
{
	...
	public unsafe class Skeletonizer : IDisposable
	{
        ...
        [DllImport("taucs.dll", CharSet = CharSet.Auto, CallingConvention = CallingConvention.Cdecl)]
		...
		{
			public Vector3d pos;
            public Vector3d normalv;
			public double radius;
			public Set<int> adjV = new Set<int>(8);      // 邻近点
		...
		// display options
        private bool displayNodePlane = false;
        ...
        public int SelectSkeNode(int Width, int Height, Vector2d currMousePosition)
        {
            {
                ...
                int minIndex = -1;
                for (int i = 0; i < simplifiedVertexRec.Count; i++)
                {
                    Vector3d v = projector.Project(simplifiedVertexRec[i].pos);
                ...
                for (int i = 0; i < simplifiedVertexRec.Count; i++) simplifiedVertexRec[i].flag = (byte)0;
                simplifiedVertexRec[minIndex].flag = (byte)1;
                return minIndex;
            }
        }
        ...
        public SkeRecord ReturnSkeleton()
        {
            if (simplifiedVertexRec == null) return new SkeRecord();
            else
            {
                ...
                for (int i = 0; i < simplifiedVertexRec.Count; i++)   // 输入rets的点集信息
                {
                    ...
                    fg[simplifiedVertexRec[i].vIndex] = i;      // 注意深复制浅复制
                    setl.Add(new Set<int>());
                }
                for (int i = 0; i < simplifiedVertexRec.Count; i++)   // 输入rets的邻近点信息
                {
                    int[] setarr = simplifiedVertexRec[i].adjV.ToArray();
                    for (int j = 0; j < setarr.Length; j++)
                    {
                        setl[i].Add(fg[setarr[j]]);
                    }
                }
                return new SkeRecord(nodePosL,setl);
            }
        }
        public Vector3d GetNodePos(int index)
        {
            return simplifiedVertexRec[index].pos;
        }
        public bool SetNodePos(Vector3d newp, int idx)
        {
            if (idx >= simplifiedVertexRec.Count) return false;
            simplifiedVertexRec[idx].pos = newp;
            return true;
        }
		public void Display()
		{
			lock (this)
			{
                ...
                if (displayNodePlane && simplifiedVertexRec != null)
                {                    
                    //DrawNodePlane();
                }
			}
...
//			PostSimplification();
        ...
        public void ComputeNodePlanarNormal()
        {
            foreach (VertexRecord rec in simplifiedVertexRec)
            {
                switch (rec.adjV.Count){
                    case 1: rec.normalv = vRec[(rec.adjV.ToArray())[0]].pos - rec.pos; break;
                    case 2: rec.normalv = vRec[(rec.adjV.ToArray())[0]].pos - 
                                          vRec[(rec.adjV.ToArray())[1]].pos; break;
                }                   
            
            }
            displayNodePlane = true;
            
        }
        
        private void DrawNodePlane()
        {
            foreach (VertexRecord rec in simplifiedVertexRec)
            {
                if (rec.adjV.Count > 0 && rec.adjV.Count <= 2)
                {
                    GL.glBegin(GL.GL_TRIANGLE_FAN);
                    GL.glVertex3d(rec.pos.x, rec.pos.y, rec.pos.z);
                    for (int i = 0; i < 9; i++)
                    {
                        Vector3d p1 = rec.normalv.Cross(
                            new Vector3d(Math.Cos(2 * Math.PI * i/ 8), Math.Sin(2 * Math.PI *i / 8), 0)
                            ).Normalize() * 0.05 + rec.pos;
